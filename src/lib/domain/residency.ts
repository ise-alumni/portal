...
export async function getResidencyStats(profiles: Profile[]): Promise<ResidencyStats> {
  try {
    ...
    // Match profiles against residency partners by company
    const partnerMatches = profiles.map(profile => {
    ...
    const notAtResidencyPartner = totalProfiles - atResidencyPartner;
    ...
    const partnerStats = partners.map(partner => {
      ...
      const percentage = totalProfiles > 0 ? Math.round((count / totalProfiles) * 100) : 0;
    ...
    return {
      ...
      residencyPercentage: totalProfiles > 0 ? Math.round((atResidencyPartner / totalProfiles) * 100) : 0,
